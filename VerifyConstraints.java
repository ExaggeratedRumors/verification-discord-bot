...
public class VerifyConstraints {
    ...
    public void genNewUserKey(GuildMemberJoinEvent event){
        try{
            TextChannel textChannel = event.getGuild().getTextChannelsByName(logChannel,true).get(0);
            String encodedKey = createKey();
            textChannel.sendMessage("Cześć <@!"+event.getMember().getUser().getId()+newMemberMessage+"\n```fix\n"+encodedKey+"\n``` \n").queue();
    ...
    public void genNewKey(MessageReceivedEvent event){
        try {
            ...
            event.getChannel().sendMessage(newKeyMessage+"\n```fix\n"+encodedKey+"\n```\n").queue();
    ...
    private String createKey() {
        try {
            KeyGenerator kg = KeyGenerator.getInstance(hashInstance);
            kg.init(256);
    ...
    private boolean compareDomain(String url, MessageReceivedEvent event){
        ...
        return url.contains(siteContains);
    }
    ...
    private void assignRole(MessageReceivedEvent event, RumorsBot api, StringBuilder sbd){
        ...
        String responseBody = scanner.useDelimiter(delimiter).next();
        GuildController gc = new GuildController(event.getGuild());
        gc.addSingleRoleToMember(event.getMember() , api.getJDA().getRolesByName(roleName,true).get(0)).queue();
        gc.setNickname(event.getMember() , responseBody.substring(responseBody.indexOf("<title>") + 7, responseBody.indexOf(siteTitle))).queue();
